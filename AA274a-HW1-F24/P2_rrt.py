...
class RRT(object):
    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for _ in range(max_iters):
            z = np.random.uniform()
            if z < goal_bias: x_rand = self.x_goal
            else: x_rand = np.random.uniform(low=self.statespace_lo, high=self.statespace_hi, size=2)
...
class GeometricRRT(RRT):
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        dis = np.linalg.norm(x2-x1)
        return x1 + (x2-x1)/dis*min(dis, eps)
        ########## Code ends here ##########
        pass
